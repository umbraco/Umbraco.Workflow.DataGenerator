...
internal sealed class WorkflowConfigurator : IWorkflowConfigurator
{
    ...
    public void SetStatus(GeneratorStatusModel value) => _keyValueService.SetValue(GetType().FullName!, JsonSerializer.Serialize(value));

    public GeneratorStatusModel GetStatus()
    {
        ...
        return JsonSerializer.Deserialize<GeneratorStatusModel>(status) ?? new();
    }
}
